module RailsEmailPreview
  class EmailsController < ::RailsEmailPreview::ApplicationController
    ...
    def add_breadcrumbs
      add_breadcrumb 'HOME', '/'
      add_breadcrumb SystemMailTemplate.model_name.human, '/system_mail_templates'
      ...
      if params[:action] == 'show'
        add_breadcrumb @preview.name
      end
    end

    def set_title
      @title = crud_title(@mail_template&.name || SystemMailTemplate.model_name.human, params[:action])
    end
    ...
    def load_preview
      ...
      #TODO: should send meodel name to params and should only preview code params when using database for
      @code = SystemMailTemplate.find_by_preview_code(params[:preview_id]).code or raise ActionController::RoutingError.new('Not Found')
